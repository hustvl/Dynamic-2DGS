...
if __name__ == "__main__":
    ...
    frame = 100
    render_poses = torch.stack([pose_spherical(angle, -15.0, 4.0) for angle in np.linspace(-180, 180, frame + 1)[:-1]], 0)
    ...
    for k in range(len(os.listdir(meshshape_save_path))):
        images.append(imageio.imread(meshshape_save_path+'/'+ str(k).zfill(5)+'.png')) 
